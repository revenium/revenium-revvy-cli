...
>
> The outcome-reporting API is documented at https://docs.revenium.io/outcomes-tracking.md — you'll add a 1-line call after each terminal action.
>
...
Each outcome can carry a monetary value, which is what unlocks ROI analysis ("we spent $X in AI + tool + human costs to deflect $Y of support cost"). See https://docs.revenium.io/outcomes-tracking.md for the outcome-reporting API.
...
- `teamId` in the query string must be the **hashed** team identifier (not the raw integer). Get it from `GET https://api.revenium.ai/profitstream/v2/api/users/me` (using your write key) — the response includes `teams[0].id` (hashed) and `tenant.id`. Use `teams[0].id` here. The same form is used in your dashboard URLs.
...
**Reference**: For the full outcome-reporting payload shape, see https://docs.revenium.io/outcomes-tracking.md. For the machine-readable API surface (useful for constructing requests programmatically), fetch https://revenium.readme.io/llms.txt.
